...
class CompaniesService:
    ...
    @classmethod
    def insert_company(cls, data):
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                sql = '''INSERT INTO companies 
                         (name_company, dba_company, fein_company, address_company, apartment_company, cityzip_company, county_company, mailing_address, cityzip_mailing, phonenumber_company, id_taxation, id_business, id_state, id_status, id_user, id_location) 
                ...
                cursor.execute(sql, (
                    ...
                    data['apartment_company'],
                    data['cityzip_company'],
                    data['county_company'],
                    data['mailing_address'],
                    data['cityzip_mailing'],
                    data['phonenumber_company'],
                    data['id_taxation'],
                    data['id_business'],
                    data['id_state'],
                    data['id_status'],
            ...
            connection.commit()
            connection.close()
            return {"success": True, "message": "Company inserted successfully."}
        except Exception as ex:
            raise CustomException(ex)
